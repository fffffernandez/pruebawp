...
async def filtrar_datos(
    ...
    try:
        ...
        # SI EL AÑO DE INICIO Y FIN SON IGUALES: filtrar solo por ese año específico
        if anio_inicio == anio_fin:
            # Filtrar por un solo año + mes/día/hora
            fecha_inicio_comp = mes_inicio * 1000000 + dia_inicio * 100 + hora_inicio
            fecha_fin_comp = mes_fin * 1000000 + dia_fin * 100 + hora_fin
            
            where_clause = f"""
                {zona_column} = 1
                AND year = %s
                AND (month * 1000000 + day_of_month * 100 + hour) >= %s
                AND (month * 1000000 + day_of_month * 100 + hour) <= %s
            """
            params = (anio_inicio, fecha_inicio_comp, fecha_fin_comp)
        else:
            # AÑOS DIFERENTES: filtrar por rango completo (año + mes + día + hora)
            fecha_inicio_comp = anio_inicio * 1000000000 + mes_inicio * 1000000 + dia_inicio * 100 + hora_inicio
            fecha_fin_comp = anio_fin * 1000000000 + mes_fin * 1000000 + dia_fin * 100 + hora_fin
            
            where_clause = f"""
                {zona_column} = 1
                AND (year * 1000000000 + month * 1000000 + day_of_month * 100 + hour) >= %s
                AND (year * 1000000000 + month * 1000000 + day_of_month * 100 + hour) <= %s
            """
            params = (fecha_inicio_comp, fecha_fin_comp)
        ...
        kpis_query = f"""
            SELECT 
                ...
                SUM(consumption_kwh) / NULLIF(COUNT(DISTINCT hour), 0) as total_por_hora,
        ...
        # Consulta para consumo por hora (SUMA total por cada hora en el período)
        ...
        # Reconstruir where_clause sin filtro de zona
        if anio_inicio == anio_fin:
            fecha_inicio_comp = mes_inicio * 1000000 + dia_inicio * 100 + hora_inicio
            fecha_fin_comp = mes_fin * 1000000 + dia_fin * 100 + hora_fin
            where_clause_sin_zona = f"""
                year = %s
                AND (month * 1000000 + day_of_month * 100 + hour) >= %s
                AND (month * 1000000 + day_of_month * 100 + hour) <= %s
            """
            ...
            fecha_fin_comp = anio_fin * 1000000000 + mes_fin * 1000000 + dia_fin * 100 + hora_fin
            where_clause_sin_zona = f"""
                (year * 1000000000 + month * 1000000 + day_of_month * 100 + hour) >= %s
                AND (year * 1000000000 + month * 1000000 + day_of_month * 100 + hour) <= %s
            """
        ...
        # Consulta para consumo y temperatura por hora (agrupado por día y hora)
        consumo_temp_hora_query = f"""
            ...
            FROM consumo_granada
            WHERE {where_clause}
            GROUP BY year, month, day_of_month, hour
